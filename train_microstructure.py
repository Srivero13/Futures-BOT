"""Frozen CPU ridge research and later-recording evaluation; never live approval."""
import argparse
import json
...
SETTINGS={'horizon_seconds':5,'max_label_lateness_ms':250,'max_depth_receipt_gap_ms':1000,'warmup_ms':1000}
...
def load_dataset(root):
    ...
    with path.open() as f:
        for line in f:
            ...
            if not 5_000_000_000<=end-start<=5_250_000_000 or (last_end is not None and start<last_end):
    ...
    x=np.array([[r[k] for k in FEATURES] for r in rows]);y=np.array([r['target_mid_return_log_bps'] for r in rows])
...
def fit(root,output):
    ...
    model={'kind':'microstructure_research_ridge','approved':False,'features':FEATURES,'settings':SETTINGS,'alpha':10,
...
def evaluate(root,model_path,output):
    ...
    if m['kind']!='microstructure_research_ridge' or m['features']!=FEATURES or m['settings']!=SETTINGS or m['alpha']!=10:
    ...
    for i in range(len(m['training_forecast_bucket_cuts'])+1):
        ...
        buckets.append({'bucket':i+1,'count':int(mask.sum()),'mean_forecast_log_bps':float(p[mask].mean()) if mask.any() else None,
            'mean_actual_log_bps_before_costs':float(y[mask].mean()) if mask.any() else None})
    ...
    atomic_json(Path(output),{'approved':False,'pnl':None,'summary':summary,'model_sha256':digest,'evaluation':source,
        ...
        'Five-second midpoint forecast diagnostics, not executable P&L. Serial dependence and selection effects remain.']})
    return summary
