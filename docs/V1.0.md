...
Python 3.10+; tested with Python 3.12 on Linux. Linux is the initial choice for the service. No GPU is required for this CPU model. Hardware and network performance must be measured on each deployment host.
...
## Measure on the deployment host
...
The second command measures REST response time, including DNS/TLS, and estimates clock uncertainty. It does not measure order acknowledgments. Profiles expire after 24 hours. Do not reuse the development profile as a deployment-host measurement.
