"""One fixed 24-hour breakout / four-hour hold hypothesis; development only."""
import argparse
...
def signal(history, decision):
    if decision % (60*MINUTE) or len(history) != LOOKBACK+1:
        return False
    rows = list(history)
    return D(rows[-1]['close']) > max(D(r['high']) for r in rows[:-1])
...
def _simulate(rows, start, end, progress):
    ...
    drawdown = D(0)
    history = deque(maxlen=LOOKBACK+1)
...
def summarize(reports):
    with localcontext() as context:
        ...
        total = sum((D(t['net_pnl']) for t in trades), D(0))
        best = max((D(t['net_pnl']) for t in trades), default=None)
        stress = sum((D(r['summary']['fixed_quantity_extra_slippage_bps_per_side']['1']) for r in reports), D(0))
        positive = sum(D(r['summary']['net_pnl']) > 0 for r in reports)
        reasons = []
        if len(trades) < 30:
            reasons.append('Fewer than 30 completed development trades')
        if positive < 4:
            reasons.append('Fewer than four positive months')
        if best is None or total-best <= 0:
            reasons.append('Not positive after removing best trade')
        if stress <= 0:
            reasons.append('Not positive with extra 1 bps slippage per side')
        return {'folds': len(reports), 'closed_trades': len(trades), 'positive_months': positive,
            'sum_independent_month_net_pnl': str(total),
            'net_pnl_excluding_best_trade': str(total-best) if best is not None else None,
            'extra_1bps_per_side_net_pnl_fixed_quantity': str(stress),
            'development_screen_failures': reasons,
            'status': 'insufficient_development_evidence' if reasons else 'requires_independent_validation_not_approved',
            'monthly': [{'month': r['test_start'][:7], **r['summary']} for r in reports]}
...
def run(paths, output):
    ...
    print('[daily-breakout] verifying development inputs', flush=True)
    ...
    protocol = {'hypothesis': 'A close beyond the prior 24-hour high may precede four-hour upward continuation',
        ...
        'decision_grid_minutes': 60, 'entry_delay_minutes': 1, 'hold_minutes': HOLD,
        ...
        'sources': sources, 'code_sha256': {name: sha256(root/name) for name in
            ('research_daily_breakout.py', 'research_cross_asset.py', 'engine_v1/dataset.py',
             'engine_v1/operations.py', 'backtest_v16.py', 'train_v15.py', 'walkforward_v16.py')}}
    atomic_json(protocol_path, protocol)
    reports = []
    for i, fold in enumerate(schedule, 1):
        print(f"[daily-breakout] fold {i}/6: {fold['calibration_end']}", flush=True)
        report = simulate(candles(paths), timestamp(fold['calibration_end']), timestamp(fold['test_end']),
            lambda bars, trades: print(f'[daily-breakout] bars={bars:,} trades={trades}', flush=True))
...
def main():
    ...
    try:
        ...
        p.exit(2, f'Daily breakout research stopped: {exc}\n')
