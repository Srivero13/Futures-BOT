...
import numpy as np
from build_aggressive_flow import FLOW_FEATURES,run as build
...
def load(root):
    ...
    if report['summary'].get('feature_set')!='aggressive_flow_v1':raise ValueError('Wrong feature set')
    rows=[json.loads(line) for line in (root/'samples.jsonl').read_text().splitlines()]
    extra=np.array([[r[k] for k in FLOW_FEATURES] for r in rows])
    if not np.isfinite(extra).all() or len(extra)!=len(y):raise ValueError('Invalid flow')
...
def compare(training,evaluation,output):
    ...
    metrics={};models={}
    for name,a,b in [('three_features',tx,x),('plus_aggressive_flow',trich,rich)]:
    ...
    summary={'training_samples':len(ty),'evaluation_samples':len(y),'zero_rmse_log_bps':float(np.sqrt(np.mean(y*y))),
        'models':metrics,'flow_minus_base_rmse_log_bps':metrics['plus_aggressive_flow']['rmse_log_bps']-metrics['three_features']['rmse_log_bps']}
    atomic_json(output,{'approved':False,'pnl':None,'summary':summary,'training':tp,'evaluation':ep,
        'weights_for_reproducibility_only':models,'extra_features':FLOW_FEATURES,'alpha':10,'runner_sha256':sha256(Path(__file__)),
        ...
        'Extra features are receipt-ordered taker notional imbalance, aggregate-event rate and flow/book interaction; not full order-book depth.',
...
def main():
    ...
    try:
        ...
        print('[flow] building training recording',flush=True);build(a.training_capture,a.output_dir/'training')
        print('[flow] building evaluation recording',flush=True);build(a.evaluation_capture,a.output_dir/'evaluation')
        print('[flow] fitting paired CPU models',flush=True)
        print(json.dumps(compare(a.output_dir/'training',a.output_dir/'evaluation',a.output_dir/'report.json'),indent=2))
    except (ValueError,OSError,KeyError,TypeError) as e:p.exit(2,f'Flow research stopped: {e}\n')
