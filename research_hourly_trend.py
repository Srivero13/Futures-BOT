"""Fixed cost-scaled hourly trend experiment. Development only; no orders."""
import argparse
...
def movement_floor():
    # Same price impact and fee convention as the existing minute simulator.
    c = Costs()
    f = c.fee_bps / 10000
    impact = (c.spread_bps / 2 + c.slippage_bps) / 10000
    return 2 * 10000 * (math.log1p(f) - math.log1p(-f)
                       + math.log1p(impact) - math.log1p(-impact))


def should_enter(x, ts, threshold):
    score = trend_score(x)
    return bool(ts % GRID_MS == 0 and score > threshold
                and float(x[2]) * 10000 > movement_floor())
...
def calibrate(rows,start,end):
    if start>=end or start%GRID_MS or end%GRID_MS:
        raise ValueError('Calibration boundaries must align with the 60-minute grid')
    ...
    for row in rows:
        ...
        if start<=decision<end and decision%GRID_MS==0 and len(history)==21:
            scores.append(trend_score(feature_matrix(list(history))[-1]))
    ...
    return {'threshold':threshold_from_scores(scores),'quantile':QUANTILE,
            'grid_examples':len(scores),'positive_trend_scores':sum(s>0 for s in scores),
...
def evaluate(rows,start,end,calibration,progress=None):
    ...
    def rule(x,ts):
        return should_enter(x, ts, threshold)
    ...
    summary['past_momentum_floor_log_bps']=movement_floor()
    summary.pop('ood_rejected')
    summary['entry_policy']='upward trend above preceding-month 95th percentile and past momentum above twice round-trip cost; no forecast gate'
    report['calibration']=calibration
    report['limitations']=[s for s in report['limitations'] if 'forecasts were trained' not in s]
    report['limitations'].extend([
        'Past movement above costs is a filter, not a forecast or evidence of positive expected net return.',
        'Costs are charged on every simulated fill; positive score does not imply expected profit above costs.',
        'One-minute entry delay, 60-minute holding period from fill, decisions on the fixed UTC 60-minute grid.',
...
def run(paths,symbol,first_test,months,reserve_from,output):
    ...
    sources=[]
    print('[hourly-trend] verifying inputs',flush=True)
    for path in paths:
    ...
    protocol={'symbol':symbol,'schedule':schedule,'reserve_from':reserve_from,
        ...
        'threshold':'max(0, calibration 95th percentile); strict greater-than entry',
        'past_momentum_floor_log_bps':movement_floor(),
        'hypothesis':'20-minute upward trend continuation over a 60-minute holding period',
    ...
    for i,fold in enumerate(schedule,1):
        print(f'[hourly-trend] fold {i}/{months}: calibration {fold["train_end"]}; test {fold["calibration_end"]}',flush=True)
        cal=calibrate(candles(paths),timestamp(fold['train_end']),timestamp(fold['calibration_end']))
        print(f'[hourly-trend] threshold={cal["threshold"]:.6f}, calibration examples={cal["grid_examples"]}',flush=True)
        report=evaluate(candles(paths),timestamp(fold['calibration_end']),timestamp(fold['test_end']),cal,
            lambda bars,trades:print(f'[hourly-trend] bars={bars:,} trades={trades}',flush=True))
        report['test_start']=fold['calibration_end'];report['test_end']=fold['test_end']
        reports.append(report)
        print('[hourly-trend] result: '+json.dumps(report['summary']),flush=True)
...
def main():
    ...
    try:
        ...
        print(f'Hourly trend experiment stopped: {error}',file=sys.stderr);return 2
