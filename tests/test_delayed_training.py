...
from train_delayed_microstructure import SETTINGS,fit,evaluate,fit_arrays,predict


class DelayedTrainingTests(unittest.TestCase):
    def dataset(self,path,start,capture):
        path.mkdir();rng=np.random.default_rng(27);rows=[]
        for i in range(200):
            imbalance=float(rng.uniform(-1,1))
            rows.append(dict(decision_monotonic_ns=i*6_000_000_000,entry_monotonic_ns=i*6_000_000_000+500_000_000,label_end_monotonic_ns=i*6_000_000_000+5_500_000_000,
                decision_wall_ns=start+i*6_000_000_000,spread_bps=1.,quantity_imbalance=imbalance,log_best_quantity=2.,target_quote_return_log_bps=imbalance*2))
